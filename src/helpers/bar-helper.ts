...
export const convertToBarTasks = (
  ...
  const groupedArray = tasks.reduce((results, obj) => {
    const existingGroup = results.find(item => item === obj.groupName);
    if (!existingGroup)
      results.push(obj.groupName);
    return results;
  }, new Array<string>());
  ...
  let barTasks = tasks.map((t) => {
    return convertToBarTask(
      t,
      groupedArray.indexOf(t.groupName),
      dates,
      dateDelta,
      columnWidth,
      rowHeight,
